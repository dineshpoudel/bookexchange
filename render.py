...
def getOneBook(id):
	book = memcache.get('book_%s'%id)
	if book is None:
		book = Book.get(db.Key.from_path('Book', int(id)))
		memcache.add('book_%s'%id, book)
	return book

def findBook(q):
	foundBook = []
	books = memcache.get('allbooks')
	if books is None or books == [] or books == '':
		books = db.GqlQuery('SELECT * FROM Book')
		books = list(books)
		memcache.add('allbooks',books)
	for book in books:
		if q.lower() in book.bookTitle.lower():
			foundBook.append(book)
	return foundBook

#google Search
def google(q):
	links = memcache.get('google_%s'%(q.lower()))
	if links is None:
		url = ("https://www.googleapis.com/customsearch/v1?key=XXXXXX&cx=011616688212487868943:hmezcysooqg&q=%s&alt=json&safe=high&searchType=image&num=10"%q)
		...
		memcache.add('google_%s'%(q.lower()) , links)	
	return links
